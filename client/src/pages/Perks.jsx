...
/*export default function Perks(selected,onChange) {
    function handleCbClick(ev) {
        //when click on it, report to function onChange,
        //onChange([...selected,name]); //onChange  return an array with all select
        const {checked, name} = ev.target;
        if (checked) {
            onChange([...selected,name]);
        } else {
    ...
    return (
        <>
                   <label className="border p-4 flex rounded-2xl gap-2 item-center cursor-pointer">
                    ...
                    <label className="border p-4 flex rounded-2xl gap-2 item-cente cursor-pointer">
